...
public class PopularProductCacheService {
    ...
    public PopularProductsCacheDto getCachedPopularProducts() {
        Pageable pageable = PageRequest.of(0, 100);
        List<PopularProductResponse> products = productRepository.findByStatusOrderBySalesCountDesc(ProductStatus.ON_SALE, pageable)
                .stream()
                .map(PopularProductResponse::from)
                .toList();
        return PopularProductsCacheDto.from(products);
    }
    ...
    public PopularProductsCacheDto warmUpPopularProductsCache() {
        Pageable pageable = PageRequest.of(0, 100);
        List<PopularProductResponse> products = productRepository.findByStatusOrderBySalesCountDesc(ProductStatus.ON_SALE, pageable)
                .stream()
                .map(PopularProductResponse::from)
